...
class ControlNet(nn.Module):
    ...
    def forward(self, x, hint, timesteps, context, **kwargs):
        ...
        # Debugging: h와 guided_hint의 크기 확인
        print(f"[DEBUG] x shape: {x.shape}, guided_hint shape: {guided_hint.shape}")
        ...
        if guided_hint.shape[-2:] != h.shape[-2:]:
            print(f"[WARNING] guided_hint 크기를 조정합니다: {guided_hint.shape[-2:]} -> {h.shape[-2:]}")
        ...
        for module, zero_conv in zip(self.input_blocks, self.zero_convs):
            ...
            # 첫 번째 블록에서 guided_hint를 추가
            if guided_hint is not None:
                h += guided_hint
                guided_hint = None  # 한 번만 추가되도록 설정
